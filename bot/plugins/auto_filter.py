...
async def auto_filter(bot, update):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("🌹ആ അതിൽ ഇല്ലേ എന്നാ ഈ ഓപ്ഷൻ ഒന്ന് നോക്ക് 🌹", callback_data=f"navigate(0|next|{query})")
                ]
            )
        ...
        try:
            await bot.send_message(
                ...
                text=f"🥰ചെക് the Found result of Ma Movies🥰...{(len_results)} Results For Your ചോദിച്ചു വാങ്ങു 🥰: <code>{query}</code>",
